...
class TestEmrJobs(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        cls.spark = SparkSession.builder             .appName("TestEmrJobs")             .master("local[*]")             .getOrCreate()
    ...
    def test_predictive_model(self, mock_psycopg2_connect):
        ...
        data = [
            ("AAPL", "2023-01-01", 150.0, 155.0, 149.0, 152.0, 100000),
        ...
        with patch('emr_jobs.predictive_model.spark.read.jdbc') as mock_read_jdbc:
            ...
            # Call the main function of the EMR job
            main()
